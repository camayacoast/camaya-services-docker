...
import { Table, Button, Input, Modal, message, Typography, Tooltip, Upload } from 'antd'
...
export default function Page(props) {
    ...
    const reservationListQuery = SalesAdminPortalService.accountList();
    ...
    const columns = [
        ...
        //     render: (text, record) => <Link to={`view-reservation/${record.reservation_number}`}><small>{text}</small></Link>,
        // },
        {
            title: 'Client #',
            // dataIndex: 'client',
        ...
        },
        {
            ...
            defaultFilteredValue: ['approved', 'for_review', 'reviewed', 'pending', 'for_client_number_encoding', 'cancelled', 'void'],
    ...
    return (
        <div>
            <div className="mt-4 mb-4" style={{display:'flex', justifyContent:'space-between'}}>
                ...
                <Input style={{width: 300}} type="search" placeholder="Search account" onChange={(e)=>handleSearch(e)} />
            </div>
            <Table
                size="small"
                columns={columns}
