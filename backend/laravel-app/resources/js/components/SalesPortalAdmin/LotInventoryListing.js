...
import {PrinterOutlined, EditOutlined, DeleteOutlined, ExclamationCircleOutlined, UserOutlined} from '@ant-design/icons'
...
const Page = (props) => {
    ...
    const subdivisionList = SalesAdminPortalService.subdivisionList('lot');
    const dashboardCounts = SalesAdminPortalService.inventoryDashoardCounts('lot');
    ...
    const handleSearchForm = (e, type) => {
        ...
        if( e != '' && type == 'subdivision' ) {
            ...
            customFilterQuery({...custom_filters, type: 'lot'}, {
                onSuccess: (res) => {
                    let data = res.data;
        ...
        updateLotDetailsQuery(values, {
            onSuccess: (res) => {
                ...
                addInventoryActivityLog({
                    type: 'lot',
                    action: 'update',
                    description: 'Lot inventory with id' + values.id + ' is updated',
                    details: values
                });
    ...
    const handleSubdivisionSelect = (e) => {
        ...
        customFilterQuery({subdivision_search: e, type: 'lot'}, {
            onSuccess: (res) => {
                let data = res.data;
                ...
                deleteLotDetailsQuery(record, {
                    onSuccess: (res) => {
                        ...
                        addInventoryActivityLog({
                            type: 'lot',
                            action: 'delete',
                            description: 'Lot inventory with id ' + record.id + ' is deleted',
                            details: record
                        });
    ...
    return (
        <>
            <div className="mt-4 mb-4" style={{display:'flex', justifyContent:'space-between'}}>
                ...
                <Space>
                    <ImportInventory property_type="lot" refetchData={refetchData} addActivityLog={addInventoryActivityLog} />
                    ...
                    <Row gutter={[8, 8]}>
                        <Col xl={12}>
                            <Form.Item name="subdivision" label="Subdivision">
                                <Select>
                                    {
                                        subdivisionList.data &&
                                            _.uniqBy(subdivisionList.data.map( i => { return { subdivision: i.subdivision, name: i.subdivision_name} }), 'subdivision')
                                            .map( (i, key) => {
                                                return <Select.Option key={key} value={i.subdivision}>{i.subdivision} {i.name ? `- ${i.name}` : ''}</Select.Option>
                                            })
                                    }
                        ...
                        <Col xl={12}>
                            <Form.Item name="price_per_sqm" label="Price per sqm (PHP)">
                                <InputNumber disabled={['reserved', 'sold'].includes(editFormData.status) && editFormData.has_reservation_agreement === true} 
            ...
            >
                <Form form={priceUpdateForm} layout="vertical" onFinish={handlePriceUpdateFinish}>
                    <Row gutter={[8, 8]}>
                            ...
                            <Form.Item rules={[
                                ...
                                <Select style={{width: '100%'}} disabled={!priceUpdateSubdivision} onChange={(e) => {
                                    ...
                                    { subdivisionOptions !== null &&
                                        ...
                                        .map( (i, key) => {
                                            if( i.subdivision == selectedUpdateSubdivision &&  i.phase == selectedUpdatePhase ) {
                                                return <Select.Option key={key} value={i.block}>{i.block}</Select.Option>
                                            }
                                        })
                                    }
                            ...
                            <Form.Item rules={[
                                ...
                                <Select style={{width: '100%'}} disabled={!priceUpdateSubdivision} onChange={(e) => {
                                    ...
                                    { subdivisionOptions !== null &&
                                        ...
                                        .map( (i, key) => {
                                            if( i.subdivision == selectedUpdateSubdivision &&  i.phase == selectedUpdatePhase &&  i.block == selectedUpdateBlock ) {
                                                return <Select.Option key={key} value={i.lot}>{i.lot}</Select.Option>
                                            }
                                        })
                                    }
                                ...
                                <Select style={{width: '100%'}} disabled={!priceUpdateSubdivision} onChange={(e) => setPriceUpdateType(e)}>
                                    { subdivisionOptions !== null &&
                                        subdivisionOptions.map( (i, key) => {
                                            if( i.subdivision == selectedUpdateSubdivision &&  i.phase == selectedUpdatePhase &&  i.block == selectedUpdateBlock &&  i.lot == selectedUpdateLot ) {
                        ...
                        <Col xl={24}>
                            <Alert class="mb-4" message={`Current price per sqm (prices will be updated): ${_.uniq(_.map(priceUpdateTotalRecords, i => i.price_per_sqm))}`} /> 
