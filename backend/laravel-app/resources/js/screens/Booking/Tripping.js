...
function Page(props) {
    ...
    const [bookingTabs, setBookingTabs] = React.useState(props.booking_tabs ? props.booking_tabs : []);
        ...
        switch (action) {
            case 'add':
                ...
                setBookingPanes(prevPanes => [...prevPanes, {
                    ...
                    content: <NewBookingComponent isTripping={true} counter={counter} paneKey={activeKey} updateBookingTabs={updateBookingTabs} setBookingPaneActiveKey={setBookingPaneActiveKey} setBookingPanes={setBookingPanes} />,
                    key: activeKey
                }]);
            break;
                ...
                setBookingPanes(prevPanes => {
                    ...
                    if (paneExists < 0) {
                        ...
                        const newPanes = [...prevPanes, {
                            ...
                            content: <ViewBookingComponent isTripping={true} bookingPaneEdit={bookingPaneEdit} referenceNumber={targetKey} />,
                            key: `${targetKey}`
                        }];
            ...
            case 'update':
                ...
                setBookingPanes(prevPanes => {
                    ...
                    prevPanes[paneExists] = {
                        title: <><span className={tagColor[status]}>&bull;</span>{` View ${targetKey}`}</>,
                        content: <ViewBookingComponent isTripping={true} updateBookingTabs={updateBookingTabs} referenceNumber={targetKey} />,
                        key: `${targetKey}`
                    };
    ...
    React.useEffect( () => {
        ...
        bookingTabs && bookingTabs.map( (booking, key) => {
            viewBooking.push({
                ...
                content: <ViewBookingComponent isTripping={true} bookingPaneEdit={bookingPaneEdit} referenceNumber={booking.reference_number} status={booking.status} />,
        ...
        const panes = [
            ...
            // },
            {
                ...
                content: (
                    <Suspense fallback={<Loading/>}><ActiveBookingsComponent isTripping={true} bookingPaneEdit={bookingPaneEdit}/></Suspense>
                ),
                key: 'active-bookings',
                closable: false,
            },
            {
                ...
                content: (
                    <Suspense fallback={<Loading/>}><UpcomingBookingsComponent isTripping={true} bookingPaneEdit={bookingPaneEdit}/></Suspense>
                ),
            ...
            },
            {
                ...
                content: (
                    <Suspense fallback={<Loading/>}><PastBookingsComponent isTripping={true} bookingPaneEdit={bookingPaneEdit}/></Suspense>
                ),
                key: 'past-bookings',
                closable: false,
            },
        ...
        if (!_.includes(['active-bookings', 'upcoming-bookings', 'past-bookings', ..._.map(bookingTabs, e => e.reference_number)], props.pane_active_key)) {
            setBookingPaneActiveKey('active-bookings');
        } else {
    ...
    React.useEffect( () => {
        props.dispatch(stateAction.updateTrippingTabs(bookingTabs));
    }, [bookingTabs]);

    React.useEffect( () => {
        props.dispatch(stateAction.updateTrippingPaneActiveKey(bookingPaneActiveKey));
    ...
    return <BookingLayout {...props}>
        <div className="fadeIn">
            ...
            <Modal
                ...
                footer={null}
                >
                    <SearchBookingComponent isTripping={true} bookingPaneEdit={bookingPaneEdit} setsearchBookingModalVisible={setsearchBookingModalVisible}/>
            </Modal>

            <Typography.Title level={2}>Tripping</Typography.Title>
...
const mapStateToProps = (state) => {
    return {
        booking_tabs: state.Booking.trippingTabs,
        pane_active_key: state.Booking.trippingPaneActiveKey
    }
}
