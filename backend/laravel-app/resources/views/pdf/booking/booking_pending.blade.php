...
<html xmlns="http://www.w3.org/1999/xhtml">
    ...
    <div class="wrapper">
        ...
        <h3 style="text-transform: uppercase; text-align: center;">BOOKING STATUS: <span style="color: orange">{{$booking['status']}}</span></h3>
        ...
        <table border="0" width="100%" style="margin-top: 32px; margin-bottom: 32px;">
                ...
                <tr>
                    <td>Customer</td>
                    <td>{{$booking['customer']['first_name']}} {{$booking['customer']['last_name']}} ({{$booking['customer']['user_type']}})</td>
                </tr>
    ...
    return $item['price'] . ' ('.$item['selling_price_type'].')';
                ...
                @if ($camaya_transportations)
                    @foreach ($camaya_transportations as $camaya_transportation)
                        <tr>
                            ...
                            <td align="left" style="text-transform: capitalize;"><small>{{$camaya_transportation['transportation']['status']}}</small></td>
                        </tr>
                    @endforeach
                @endif
        </table>
        ...
        @if (collect($booking['invoices'])->sum('balance') > 0)
            <div class="page-break-before"></div>
            @include('pdf.booking.components.payment_instructions', ['booking' => $booking, 'camaya_transportations' => $camaya_transportations])
        @endif
