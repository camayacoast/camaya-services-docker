...
export default function Page(props) {
    ...
    // Queries
    const lotInventoryList = SalesAdminPortalService.condoInventoryList('condo');
    ...
    const [exportInventoryStatusReportQuery, { isLoading: exportInventoryStatusReportQueryIsLoading, reset: exportInventoryStatusReportQueryReset}] = SalesAdminPortalService.exportCondoInventoryStatusReport();
    ...
    const getAreaList = () => {
        let data = [];
        if (lotInventoryList.data) {
            data = _.sortBy(_.uniqBy(_.map(lotInventoryList.data, i => { return { value: i.area, text: i.area} }), 'value'), 'value');
        } else {
            data = [];
        }
        return data;
    ...
    const handleDeleteButtonClick = (record) => {
        Modal.confirm({
            ...
            content: 'Are you sure you want to delete condominium inventory record?',
            okText: 'OK',
            cancelText: 'Cancel',
            onOk: () => {
                ...
                deleteLotDetailsQuery(record, {
                    onSuccess: (res) => {
                        notification.success({
                            message: 'Condominium record successfully deleted!',
                            description:
                              '',
                          });
    ...
    const editFormOnFinish = (values) => {
        ...
        values.price_per_sqm = editFormData.price_per_sqm;
    ...
    return (
        <>
            ...
            <div className="mt-4 mb-4" style={{display:'flex', justifyContent:'space-between'}}>
                ...
                <Space>
                    <ImportInventory property_type="condo" lotInventoryList={lotInventoryList} addActivityLog={addInventoryActivityLog} />
            ...
            >
                <Form form={editForm} layout="vertical" onFinish={editFormOnFinish}>
                    ...
                    <Row gutter={[8, 8]}>
                        ...
                        <Col xl={12}>
                            <Form.Item disabled={['reserved', 'sold'].includes(editFormData.status)} name="area" label="Area" rules={[{ required: true }]}>
                                <InputNumber 
            ...
            <Card size="small">
                Search:
                <Space>
                    <Select style={{ width: 300, marginLeft: 16 }} placeholder="Project" value={selectedSubdivision} onChange={e => {
                    ...
                    <Select style={{width: 150}} onChange={e => setSelectedLot(e)} value={selectedLot}>
                        <Select.Option value=""></Select.Option>
                        {
                            ...
                            .map( (i, key) => {
                                return <Select.Option key={key} value={i.lot}>Unit {i.lot}</Select.Option>
                            })
                        }
                    </Select>
                </Space>
