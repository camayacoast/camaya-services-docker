...
export default function Page(props) {
    ...
    const [getSDMBGolfPlayConsumptionQuery, { isLoading: getSDMBGolfPlayConsumptionQueryIsLoading, reset: getSDMBGolfPlayConsumptionQueryReset }] = BookingReportService.getSDMBGolfPlayConsumption();
    ...
    const handleGo = () => {
        ...
        getSDMBGolfPlayConsumptionQuery({
    ...
    const handleDownload = () => {
        ...
        saveAs(new Blob([s2ab(wbout)],{type:"application/octet-stream"}), 'SDMB-Golf-Play.xlsx');
    }

    return (
        <>
            <Typography.Title level={4}>Golf Play Consumption</Typography.Title>
            ...
            <Button onClick={()=>handleGo()} className="ml-2" type="primary" loading={getSDMBGolfPlayConsumptionQueryIsLoading}>Go</Button>
            <Button onClick={()=>handleDownload()} className="ml-2" type="primary">Download</Button>

            <Typography.Paragraph>
                <strong>SDMB - Golf Play Report</strong><br/>
                Only those confirmed bookings with SDMB Booking tag that availed packages with GOLFPLAY code will reflect on this report
            </Typography.Paragraph>

        { data.length ?
            <>{
                (getSDMBGolfPlayConsumptionQueryIsLoading) ?
                ...
                :
                    ...
                    <table id="table">
                        <thead>
                            <tr>
                                    <th colSpan={dateRange.length + 2} style={{backgroundColor: 'rgb(112,173,71)'}} align="center" >Golf Play Consumption</th>
                                </tr>
                                <tr>
                                    ...
                                    return <tr key={key}>
                                        ...
                                        </td>
                                        {
                                            item.consumptions && Object.values(item.consumptions).map( (consumption, key2) => {
                                                return <td key={key2} style={{padding: 5, border: 'solid 1px gainsboro'}} align="center">
                                                    {numberWithCommas(consumption.golf_play_rates || 0)}
                                                </td>
                                            })
                                        }
                                        <td style={{padding: 5, border: 'solid 1px gainsboro'}} align="center">
                                            { numberWithCommas(_.sumBy(Object.values(item.consumptions), 'golf_play_rates')) }
                                        </td>
                                    </tr>
                                })
                            }
