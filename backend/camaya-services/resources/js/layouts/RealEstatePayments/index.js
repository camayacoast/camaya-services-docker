...
function RealEstatePayments(props) {

    const [collapsed, setCollapsed] = React.useState(true);
    const [collapsedWidth, setCollapsedWidth] = React.useState(115);
    ...
    React.useEffect( () => {
        ...
        switch (props.location.pathname) {
            ...
            case '/real-state-payments/aaa':
                setselectedMenu('/real-state-payments/aaa');

            default:
                setselectedMenu('/real-state-payments');
        }
            ...
            >
                <div style={{display: 'flex', justifyContent:'space-between', alignItems:'center', flexDirection:'column', height: '100%'}}>
                    <div className="logo" style={{textAlign: 'center', color: 'white'}}>
                        <div>Real Estate Payments</div>
                    </div>
                    <Menu
                        theme="dark"
                        mode="inline"
                        defaultSelectedKeys={['/real-estate-payments']}
                        selectedKeys={[selectedMenu]}
                    >
                        <Menu.Item key="/" icon={<ArrowLeftOutlined />}>
                            <NavLink to="/">Back to Main</NavLink>
                        </Menu.Item>
                        <Menu.Item key="/real-state-payments" icon={<DashboardOutlined />}>
                            <NavLink to="/real-state-payments">Dashboard</NavLink>
                        </Menu.Item>
                    </Menu>
...
export default connect(mapStateToProps)(withRouter(RealEstatePayments))
