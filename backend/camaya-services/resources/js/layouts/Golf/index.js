...
function Golf(props) {
    ...
    React.useEffect( () => {
        ...
        switch (props.location.pathname) {
            ...
            case '/golf/tee-time':
                setselectedMenu('/golf/tee-time');

            default:
                setselectedMenu('/golf');
        }
            ...
            >
                <div style={{display: 'flex', justifyContent:'space-between', alignItems:'center', flexDirection:'column', height: '100%'}}>
                    <div className="logo" style={{textAlign: 'center', color: 'white'}}>
                        <Icon component={GolfBallSolid}/>
                        <div>Golf</div>
                    </div>
                    <Menu
                        theme="dark"
                        mode="inline"
                        defaultSelectedKeys={['/golf']}
                        selectedKeys={[selectedMenu]}
                    >
                        <Menu.Item key="/" icon={<ArrowLeftOutlined />}>
                            <NavLink to="/">Back to Main</NavLink>
                        </Menu.Item>
                        <Menu.Item key="/golf" icon={<DashboardOutlined />}>
                            <NavLink to="/golf">Dashboard</NavLink>
                        </Menu.Item>
                        <Menu.Item key="/golf/tee-time" icon={<FieldTimeOutlined />}>
                            <NavLink to="/golf/tee-time">Tee Time</NavLink>
                        </Menu.Item>
                    </Menu>
...
export default connect(mapStateToProps)(withRouter(Golf))
